...
### 1. 手順書の作成・AI使わないモード (NEW ⚡)
* **AIによる動画・音声解析**: アップロードされた動画から音声文字起こしと操作手順のステップ化を自動実行。
* **AIを使わない手動モード (直ちに編集画面へ遷移) (NEW ⚡)**:
  * 動画アップロード画面に「手順書を自動生成する」の他に **「編集画面へ遷移」** ボタンを設置。
  * AIの自動解析を介さず動画のみを保存し、即座に編集画面へ直接移動して動画からの手動静止画切り出しやマニュアル作成を行うことが可能。
* **日本語出力の強制指示 (NEW 🌐)**: 動画内の音声言語に関わらず、すべてのマニュアルテキストを必ず自然な日本語で生成。
* **暗号化 HttpOnly Cookie による複数 API キー管理 & 切り替え (NEW 🔑)**:
  * 無料枠／有料枠、個人／チーム権限に応じた複数の Gemini API キーを用途ラベル（例: `無料枠`, `有料プラン`）付きで登録・保存。
  * **Fernet (AES-128)** アルゴリズムで暗号化し、ブラウザの `HttpOnly Cookie` に保存。クライアント側に生のキーを一切露出させない安全設計。
  * ヘッダーのボタンからワンクリックで使用する API キーを即座に切り替え可能。
* **Gemini ListModels 連携による利用可能モデル動的取得・突合 (NEW 🤖)**:
  * APIキー切り替え時に Gemini の `ListModels` API をリアルタイム呼び出し、そのキーで利用可能なモデル（`Gemini 3.5 Flash`, `Gemini 3.6 Flash`, `Gemini 3.5 Flash Lite`, `Gemini 3 Pro Preview`, `Gemini 2.5 Pro`）を動的制御。
  * 利用不可のモデルはカード選択を非活性化し「(利用不可)」バッジを表示。
* **静止画の自動切り出し**: OpenCV を活用し、AIが推奨するキーフレーム（タイムスタンプ）から静止画を自動抽出し、手順書へ埋め込み。
* **プロンプトカスタマイズ**: 生成時に「初心者に分かりやすく」「安全確認を重視」などの追加指示を設定可能。

### 2. 手順書の編集・リアルタイムプレビュー & 作業領域拡張
...
* **Markdown エディタ作業領域拡張トグル (NEW 🎛)**:
  * エディタツールバーの「AI補正」および「プレビュー」ボタンでパネルを折りたたむことができ、作業領域を縦横に拡大・最大化可能。
* **テキスト & 画像の自由な編集**:
...
* **AIプロンプト指示による本文修正・補正 (NEW ✨)**:
  * 「⚠️ 注意点を追加」「🔰 初心者向けに平易化」「📌 箇条書き整理」「🔍 誤字脱字校正」などのプロンプト指示を入力することで、AIが画像タグや構造を保ったまま手順書本文をリアルタイムにリライト・補正。コンボボックスで使用AIモデルを切り替え可能。
